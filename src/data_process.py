...
class DataGenerator(object):
    ...
    def read_file(self):
        ...
        seqs_by_student = {}
        num_skills = 0  # 统计知识点的数量，之后输入的向量长度就是两倍的知识点数量
        with open(self.fileName, 'r') as f:
            for line in f:
                fields = line.strip().split()  # 一个列表，[学生id，知识点id，答题结果]
                student, problem, is_correct = int(fields[0]), int(fields[1]), int(fields[2])
                num_skills = max(num_skills, problem)  # problem实际上是用该题所属知识点来表示的
                seqs_by_student[student] = seqs_by_student.get(student, []) + [[problem, is_correct]]  # 保存每个学生的做题信息
    ...
    def gen_attr(self, is_infer=False):
        ...
        """
        if is_infer:
            seqs_by_students, num_skills = self.read_file()
    ...
    def format_data(self, seqs):
        ...
        x_sequences = np.array([[(j[0] + self.num_skills * j[1]) for j in i[:-1]] for i in seqs])
        ...
        target_id_seqs = np.array([[j[0] for j in i[1:]] for i in seqs])
